...
export const attachDevices = (t: TFunction) =>
  t(
    'ceph-storage-plugin~Selected nodes are based on the selected StorageClass. The selected nodes will preferably be in 3 different zones with a recommended requirement of 14 CPUs and 34 GiB per node.',
  );
export const attachDevicesWithArbiter = (t: TFunction) =>
  t(
    'ceph-storage-plugin~Selected nodes are based on the selected StorageClass. 4 nodes in 2 different zones will be selected with a recommended requirement of 14 CPU and 34 GiB RAM per node.',
  );
export const encryptionTooltip =
  'The storage cluster encryption level can be set to include all components under the cluster (including StorageClass and PVs) or to include only StorageClass encryption. PV encryption can use an auth token that will be used with the KMS configuration to allow multi-tenancy.';
